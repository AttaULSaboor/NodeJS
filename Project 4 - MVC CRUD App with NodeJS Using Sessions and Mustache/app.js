...
const logRequest = (req, res, next) => {
        const now = new Date();
        const data = `${now}, ${req.path}, ${req.ip}, ${JSON.stringify(req.query)}, ${JSON.stringify(req.body)}\n`;
...
// Middleware to restrict access to the editors page
function restrictEditor(req, res, next) {
        if (req.session.level && req.session.level === 'editor') {
          next();
        } else {
          res.redirect('/home');
        }
...
// Apply the middleware to the Editors route
app.use("/editors", restrictEditor, require("./controllers/editors"));
